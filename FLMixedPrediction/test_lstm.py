...
# 读取风场数据
wind_file = data_dir / 'wind_farms' / 'Wind farm site 1 (Nominal capacity-99MW).xlsx'
...
# 读取光伏电站数据
solar_file = data_dir / 'solar_stations' / 'Solar station site 1 (Nominal capacity-50MW).xlsx'
...
# 绘制损失曲线
def plot_loss(train_losses, val_losses, site_type, site_name):
...
def main():
    ...
    print(f'Wind model saved: {wind_model_path}')
    
    # 绘制风能模型结果
    plot_loss(wind_train_losses, wind_val_losses, 'wind', 'site_1')
    ...
    print(f'Solar model saved: {solar_model_path}')
    
    # 绘制光伏模型结果
    plot_loss(solar_train_losses, solar_val_losses, 'solar', 'site_1')
    ...
    print(f'\nAll results saved in: {figure_dir}')
