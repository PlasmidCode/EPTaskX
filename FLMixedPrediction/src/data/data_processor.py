...
class RenewableEnergyDataProcessor:
    ...
    def build_features(self, df: pd.DataFrame, target_col: str = 'power', seq_len: int = 24, horizon: int = 1) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for i in range(seq_len, len(df) - horizon + 1):
            ...
            # 获取目标值（多步预测）
            y_seq = df[target_col].iloc[i:i + horizon].values
    ...
    def split_time_series(self, df: pd.DataFrame, val_days: int = 30, test_days: int = 30) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        """
        时间序列数据切分（避免数据泄露）
        ...
        if time_col is None:
            ...
            test_samples = test_days * 96  # 96个15分钟步长/天
            val_samples = val_days * 96
            
            train_df = df.iloc[:-val_samples-test_samples]
            val_df = df.iloc[-val_samples-test_samples:-test_samples]
