...
class TransformerScheduler(BaseScheduler):
    ...
    def __init__(self, problem, episodes=5000, lr=1e-4, gamma=0.95, clip_eps=0.1, value_coef=0.3, entropy_coef=0.05):
        ...
        # 初始化网络
        self.policy = TransformerPolicyNetwork(problem.D, problem.T, len(problem.tasks))
        self.optimizer = optim.Adam(self.policy.parameters(), lr=lr)
    
    def solve(self):
        """
        使用 Transformer 求解调度问题
        """
        D = self.problem.D
        T = self.problem.T
        J = self.J
    ...
    def _step(self, state, action):
        ...
        if j < J and remaining_work[j] > 0 and t < T:
            ...
            # 计算迁移惩罚
            if d != current_domains[j]:
                reward -= 0.1 * self.problem.tasks[j]['S_j'] / 100  # 迁移惩罚
