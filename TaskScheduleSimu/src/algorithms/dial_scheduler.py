...
class DIALScheduler(BaseScheduler):
    """
    基于 DIAL（Deep Intention-Aware Communication）的调度算法
    """
    def __init__(self, problem, episodes=5000, lr=1e-4, gamma=0.95, clip_eps=0.1, value_coef=0.3, entropy_coef=0.05, communication_rate=0.1):
        """
        初始化 DIAL 调度器
        ...
        # 初始化网络（每个任务一个智能体）
        self.policies = [DIALPolicyNetwork(problem.D, problem.T, self.J) for _ in range(self.J)]
    ...
    def solve(self):
        ...
        for episode in range(self.episodes):
            ...
            while not done:
                ...
                # 通信阶段
                self._communicate(states)
                
                for j in range(J):
                    action, log_prob, value = self.policies[j](states[j], self.communication_buffer)
            ...
            # 更新策略
            for _ in range(4):  # DIAL 多次更新
                total_loss = 0
                
                for j in range(J):
                    # 前向传播
                    action_probs, values = self.policies[j].forward(all_states[j].detach(), self.communication_buffer)
        ...
        for t in range(T):
            ...
            for j in range(J):
                ...
                # 通信阶段
                self._communicate([state for _ in range(self.J)])
                
                # 获取动作
                action_probs, _ = self.policies[j].forward(state.unsqueeze(0), self.communication_buffer)
    ...
    def _communicate(self, states):
        ...
        for j in range(self.J):
            state = states[j]
            # 提取意图信息
            intention = state[1:3]  # 剩余工作量和当前域
            self.communication_buffer.append(intention.detach().numpy())

class DIALPolicyNetwork(nn.Module):
    """
    DIAL 策略网络
    """
    def __init__(self, D, T, J):
        ...
        J: 任务数量
        """
        super(DIALPolicyNetwork, self).__init__()
        ...
        action_dim = D  # 选择一个域
        # 通信维度
        communication_dim = 2  # 剩余工作量和当前域
        
        # 网络结构
        self.fc1 = nn.Linear(state_dim + communication_dim * J, 64)
        self.fc2 = nn.Linear(64, 64)
        self.fc_pi = nn.Linear(64, action_dim)
        self.fc_v = nn.Linear(64, 1)
        self.relu = nn.ReLU()
    
    def forward(self, x, communication_buffer):
        """
        前向传播
        """
        # 合并状态和通信信息
        communication_info = torch.tensor(np.concatenate(communication_buffer), dtype=torch.float32)
        if len(x.shape) == 1:
            x = torch.cat([x, communication_info])
        else:
            communication_info = communication_info.unsqueeze(0).repeat(x.shape[0], 1)
            x = torch.cat([x, communication_info], dim=1)
    ...
    def __call__(self, state, communication_buffer):
        """
        获取动作、对数概率和价值
        """
        # 确保状态是 2D 张量
        ...
        pi, v = self.forward(state, communication_buffer)
